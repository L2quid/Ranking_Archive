...
with open( filePath, 'w+',encoding='utf-8') as file:
    for i in range(20) :
        file.write('<div class="wrap"><div class="rank">'+str(i + 1) + '위</div><img src="'+images[i+1]+'"><div class="title"> ' + items[i+1].get_text(strip = True)+'</div><div class="title">'+name[i+1].get_text(strip = True)+'</div><div class="title">'+price[i+1].get_text(strip = True)+'</div></div><br>')
